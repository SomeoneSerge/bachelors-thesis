...
The space \( \mathtt{Mat}_{n{\times}n}(\mathbb{K})\sim L(\mathbb{K}^n) \)
...
Finally we will also be dealing with the isomorphic spaces
    \( L(L(\mathbb{K}^n)) \) and \( L(\mathtt{Mat}_{n{\times}n}(\mathbb{K})) \)
    with the operator norm.
We will follow Krein
    and refer to elements of \( L(\mathtt{Mat}_{n{\times}n}(\mathbb{K})) \)
    as ``transformers''.
...
\begin{itemize}
    \item \( J, \Gamma \in L(\mathtt{Mat}_{n{\times}n}(\mathbb{K})) \)
        are transformers;
