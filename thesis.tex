...
Suppose \( A \) has a simple spectrum,
    ...
    Then a tuple \( (\mathtt{Mat}_{{MN}{\times}{MN}}(\mathbb{K}), J, \Gamma) \)
...
This direct-sum decomposition of \( \mathbb{K}^{MN} \)
    corresponds to a decomposition of the identity matrix \( E\in \mathtt{Mat}_{MN{\times}MN}(\mathbb{K}) \)
...
For any matrix \( X\in \mathtt{Mat}_{MN{\times}MN}(\mathbb{K}) \)
...
\begin{thm}\label{nk:thm:kron}
    Consider the perturbed matrix~\eqref{-kronperturb}
        \[
            A{\otimes}B - F.
        \]
    Let \( A\in\mathtt{Mat}_{N{\times}N}(\mathbb{K}) \) and \( B\in\mathtt{Mat}_{M{\times}M}(\mathbb{K}) \)
    ...
    for some \( X^o \in \mathtt{Mat}_{MN{\times}MN}(\mathbb{K}) \),
    \( \|X^o - F\|\leq 3\|F\| \).
